...
Create the package by matching the current `aurora_chat` package layout and
tooling.
...
Use the versions currently declared by `aurora_chat`:
...
`aurora_chat`: `useComputeStore`, `useCompositionStore`, `useRecordStore`,
...
Match `aurora_chat` and normalize only the configured API base URL with
...
`{ apiBaseUrl: string }`. Match `aurora_chat`: each client command creates
