...
import { dataComposition, optionsComposition, statusComposition, submitComposition } from "./CompositionWorker";
...
export function createCompositionWorkerClient(config: WorkerConfig): CompositionWorkerClient {
  return {
    ...
    options: (token, session) => optionsComposition(config.apiBaseUrl, token, session),
  };
}
