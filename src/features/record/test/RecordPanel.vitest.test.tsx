...
describe("RecordPanel", () => {
  ...
  it("adds a separate status row for each downloaded record resource", async () => {
    ...
    const document = within(panel).getByRole("button", { name: "Recorded Document" });
    expect(within(panel).queryByText("|", { exact: true })).not.toBeInTheDocument();
    fireEvent.click(document);
    const documentDownloading = await screen.findByText("Downloading recorded document");
    expect(documentDownloading.closest("li")).toHaveAttribute("data-phase", "started");
    expect(within(panel).getByRole("button", { name: "Recorded Document" })).toBeDisabled();
    expect(within(panel).getByRole("button", { name: "Cover Page (Receipt)" })).toBeDisabled();
    fireEvent.click(screen.getByRole("button", { name: "Complete Recorded Document" }));
    await waitFor(() => expect(screen.getByText("Recorded document downloaded")).toBeInTheDocument());
    const documentDownloaded = screen.getByText("Recorded document downloaded").closest("li");
